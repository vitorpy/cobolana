...
describe('Hello Cobolana - COBOL on Solana', () => {
  ...
  beforeAll(async () => {
    ...
    console.log('🔨 Building Cobolana (COBOL on Solana)...');
    ...
    const programPath = path.join(__dirname, '..', 'zig-out', 'lib', 'cobolana.so');

    if (!fs.existsSync(programPath)) {
      throw new Error(`Program not found at ${programPath}. Run './build.sh' first.`);
    }
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (programAccount && programAccount.executable) {
        programReady = true;
        console.log('✅ COBOL program deployed successfully!');
        break;
      }
  ...
  it('should execute and log "Hello Cobolana"', async () => {
    ...
    console.log('🚀 Sending transaction to COBOL program...');
    ...
    // Check for "Hello Cobolana" in logs
    const hasMessage = logs.some(log =>
      log.includes('Hello Cobolana')
    );

    expect(hasMessage).toBe(true);

    if (hasMessage) {
      console.log('🎉 SUCCESS! COBOL is running on Solana!');
    }
  });
