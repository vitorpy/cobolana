...
#define	CB_FLAG_GETOPT_DEPEND_OUTPUT        20
#define	CB_FLAG_GETOPT_DEPEND_TARGET        21
#define	CB_FLAG_GETOPT_DEPEND_ESCAPE_TARGET 22
#define	CB_FLAG_GETOPT_DEPEND_OUTPUT_FILE   23
...
static const struct option long_options[] = {
	...
	{"Q",			CB_RQ_ARG, NULL, 'Q'},
	{"A",			CB_RQ_ARG, NULL, 'A'},
	{"M",                   CB_NO_ARG, NULL, CB_FLAG_GETOPT_DEPEND_OUTPUT },
	{"MT",			CB_RQ_ARG, NULL, CB_FLAG_GETOPT_DEPEND_TARGET},
	{"MQ",			CB_RQ_ARG, NULL, CB_FLAG_GETOPT_DEPEND_ESCAPE_TARGET},
	{"MF",			CB_RQ_ARG, NULL, CB_FLAG_GETOPT_DEPEND_OUTPUT_FILE},
...
cobc_main_strdup (const char *dupstr)
{
	...
	/* LCOV_EXCL_START */
	if (!dupstr) {
		/* HACK for cobolana-cc: return empty string instead of aborting */
		cobc_err_msg (_("warning: call to %s with NULL pointer, using empty string"), "cobc_main_strdup");
		return cobc_main_strdup("");
	}
	/* LCOV_EXCL_STOP */
	n = strlen (dupstr);
...
{
	if (!ec_list || !(*ec_list)) {
		/* NULL check for safety - turn all globally */
...
static void
add_depend_target (const char *s)
{
	if (!cb_depend_target) {
		cb_depend_target = cobc_strdup (s);
	} else {
...
process_command_line (const int argc, char **argv)
{
	...
	cb_default_byte = -1;
	cb_mf_files = 0;
	set_warn_opt_value (cb_warn_unsupported, COBC_WARN_AS_ERROR);
...
#define no_asm_msg _("the used C compiler is known to not be able to generate assembler code")
		...
		case CB_FLAG_GETOPT_DEPEND_OUTPUT: /* -M */
			...
			cb_compile_level = CB_LEVEL_PREPROCESS;
			break;
		case CB_FLAG_GETOPT_DEPEND_ON_THE_SIDE: /* -MD */
			cb_depend_output = 1;
			cb_depend_target_auto = 1;
			break;
		case CB_FLAG_GETOPT_DEPEND_ADD_PHONY: /* -MP */
			cb_depend_add_phony = 1;
			break;
		case CB_FLAG_GETOPT_DEPEND_KEEP_MISSING: /* -MG */
			cb_depend_keep_missing = 1;
			break;
		case CB_FLAG_GETOPT_DEPEND_TARGET: /* -MT <target> */
			add_depend_target (cob_optarg);
			break;
		case CB_FLAG_GETOPT_DEPEND_ESCAPE_TARGET: /* -MQ <target> */
...
#define	CB_ERRWARNDEF(opt,name,doc)	set_warn_opt_value (opt, COBC_WARN_ENABLED);
#include "warning.def"
#undef	CB_WARNDEF
#undef	CB_ONWARNDEF
#undef	CB_NOWARNDEF
...
#undef	CB_ERRWARNDEF
	...
	if (output_name && string_is_dash (output_name)) {
	...
	if (cb_depend_filename) {
		if (string_is_dash(cb_depend_filename)) {
			...
			if (!cb_depend_file) {
				cb_perror (0, "cobc: %s: %s", cb_depend_filename, cb_get_strerror ());
			}
		}
	}
...
#undef	CB_ERRWARNDEF
	...
	/* DISABLED for cobolana-cc - we don't need exception setup for sBPF output */
...
main (int argc, char **argv)
{
	...
	*/
	...
	/* HACK for cobolana-cc: bypass broken getopt and hardcode input file */
	if (iargs == argc && argc >= 2) {
		/* Assume argv[1] is the input file */
		iargs = 1;
		cb_compile_level = CB_LEVEL_TRANSLATE;  /* Just translate to sBPF */
		cobc_set_source_format (CB_FORMAT_FREE);  /* Assume free format */
		save_all_src = 1;  /* Keep generated files */
		save_temps = 1;  /* Keep temporary files */
		output_name = cobc_main_strdup("cobolana.s");  /* Output to cobolana.s */
	}
